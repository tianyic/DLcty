...
public class cMLP {
	...
	public cMLP(RealMatrix X,int inputSize,int[] hiddenSizes,int outputSize, boolean needPretrain,double lr,int batch_size,int pre_training_epoches,double alpha, int K ){
		...
		if(needPretrain==false){
			...
			cDBN cdbn=new cDBN(inputSize,hiddenSizes,outputSize,K,pre_training_epoches,alpha);
			
			cdbn.pretraining(X, batch_size);
			
			for(int i=0;i<this.num_sigmoidlayers;i++){
			
				RealMatrix weight=cdbn.getCrbmlayers().get(i).getW();
				RealMatrix bias=cdbn.getCrbmlayers().get(i).getHbias();
				this.weights.put(i, weight);
				this.bias.put(i, bias);
			}
